...
# Example 1
# Load the dataset
housingData <- read.csv('housing_price_dataset.csv')
...
lm_model <- lm(Price ~ ., data = housingTrain)
...
tree_model <- rpart(Price ~ ., data = housingTrain, method = 'anova')
...
# Building the linear regression model
lm_model <- lm(price ~ horsepower + citympg + highwaympg, data = train_data)
...
tree_model <- rpart(price ~ horsepower + citympg + highwaympg, data = train_data, method = 'anova')
rpart.plot(tree_model)
